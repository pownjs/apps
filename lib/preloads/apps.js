...
const bootAPI = () => {
    ...
    window.addEventListener('DOMContentLoaded', async() => {
        ...
        await Promise.all([
            injectRemoteScript(`https://extension.secapps.com/index.page.js?_source=${source}`) // TODO: remove depenncy of this script
        ])

        const { GenericChannel, ChannelProxy } = require('@pown/channel')

        const windowChannel = new GenericChannel(window)
        ...
        const channelProxy = new ChannelProxy(windowChannel, ipcChannel)

        channelProxy.listen('makeRequest(f7f34d94-67be-4dad-b297-9fd890dd25ba)')
        channelProxy.listen('observeTransactions(a34a46f4-fd1d-4f8e-94fc-69ed030a6fe3)')
    })
}
