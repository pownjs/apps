...
const { isFrameless } = require('./config')
...
// the browser manager is responsible for creating and managing windows and views
...
const browserManager = new class extends Manager {
    ...
    changeUri(uri) {
        if (this.useWindows) {
            ...
            uri.query = querystring.stringify(Object.assign(querystring.parse(uri.query), { _topBar: isFrameless ? true : false }))
...
const createMenu = () => {
    ...
    const tools = {
        ...
        [`Debug Chrome (port 9222)`]: async() => {
            await launch({ xssAuditor: true, certificateErrors: false, debuggingPort: 9222, launcher: chromeLauncher })
        },

        [`Debug Chrome (port 9223)`]: async() => {
            await launch({ xssAuditor: true, certificateErrors: false, debuggingPort: 9223, launcher: chromeLauncher })
        },

        [`Debug Chrome (port 9224)`]: async() => {
            await launch({ xssAuditor: true, certificateErrors: false, debuggingPort: 9224, launcher: chromeLauncher })
        },

        [`Debug Chrome (port 9225)`]: async() => {
            await launch({ xssAuditor: true, certificateErrors: false, debuggingPort: 9225, launcher: chromeLauncher })
        },
